...
def subgradient_update_mask_node_only(model, args):  
    """只更新节点掩码的子梯度"""  
...
def get_each_mask(tensor, thre):  
    """根据阈值生成二进制掩码"""  
    return (tensor.abs() > thre).float()  
  
def get_final_mask_epoch_node_only(model, rewind_weight, args):  
    """获取节点掩码的最终剪枝结果"""  
